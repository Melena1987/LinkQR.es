...
export const Preview: React.FC<PreviewProps> = ({ config }) => {
  ...
  const handleDownloadPng = async () => {
     await downloadQR(config);
  };
  
  const handleDownloadSvg = () => {
    // Basic SVG download from the DOM element currently rendered
        ...
        <div className="p-8 flex flex-col items-center justify-center bg-white min-h-[400px] relative overflow-hidden">
            ...
            <div className="relative z-10 transition-all duration-300 transform hover:scale-[1.02]">
                {renderFrameContainer(
                   <div id="preview-qr-wrapper" className="relative">
                       <QRCodeSVG 
                          config={config} 
                          className="w-[220px] h-auto"
                          style={{ borderRadius: config.dotStyle === 'dots' ? '0' : '0' }}
                       />
                       {/* Note: ID used for SVG download selector */}
                       <div id="preview-qr-svg" className="hidden">
